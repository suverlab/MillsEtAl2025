...
def importMat(file_path = const.matDirectory):
    ...
    for ind in range(len(list_values[0,:])):
        ...
        stimulus.append(expt[11][0])
        if const.TRIM:
            pufferSignal.append(np.delete(expt[12], trim, axis=0))
            tachometerSignal.append(np.delete(expt[13], trim, axis=0))
            photodiodeSignal.append(np.delete(expt[14], trim, axis=0))
        else:
            pufferSignal.append(expt[12][:])
            tachometerSignal.append(expt[13][:])
            photodiodeSignal.append(expt[14][:])
