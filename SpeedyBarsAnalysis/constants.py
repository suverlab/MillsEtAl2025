...
baseDirectory = '/Users/millskm/Desktop/Vanderbilt/SuverLab - Analysis/SpeedyBarsAnalysis/' # Main Folder
matDirectory = '/Users/millskm/Desktop/Vanderbilt/SuverLab - Analysis/SpeedyBarsAnalysis/mat/' # Folder in which all .mat files are placed
...
config_path['dorsal'] = DLCDirectory + 'fancyRig-DLC-2024-09-30/config.yaml'
...
if STIM == True: 
    ...
    if BASELINE == True:
        ...
        baselineEnd = baselineStart + baselineLen
    ONE_STIM = True # Set to true if only one stimulus PERIOD (i.e. tonic opto), will take avg over stim period and include in dataframe
    if ONE_STIM == True:
        stimStart = 2 # set to start time of stimulus period (If averaging over part of stimulus period, use the avging time)
        stimLen = 6 # set to length time of stimulus period (If averaging over part of stimulus period, use the avging time)
        stimEnd = stimStart + stimLen
        MULT_STIM_TYPES = True # Set to True if there are multiple stimulus types in which you want a conditional average for
        if MULT_STIM_TYPES == True:
            stimNames = ['0','5','10','15','20','25','30','35','40','45','50','100'] # Set to names of stimuli ### IMPLEMENT CODE ###
            stim_mat_label = 'stimulus' # Name of .mat column giving stimulus information
            stimNamesInMat = [0,5,10,15,20,25,30,35,40,45,50,100] # Set to corresponding number/label in .mat file, match with above
    elif ONE_STIM == False:
        ...
        stimNames = []
        MULT_SEQ = False # Set to true if there are multiple stimulus sequence types (i.e ramping up OR ramping down) ##ADVANCED##
        if MULT_SEQ == True:
            seq_order_name = 'stimType' # the name of sequence order in .mat file!
            stimSeqNames = [] # set to names of each sequence type
            stimSeqNumbers = [] # set to corresponding number/label in .mat file, must match dictionary order and stimSeqNames
            stimSeqs = {
            }
...
if FLIGHT == True:
    ...
    PRE_FILTERED = False # Set to true if filtering of tachometer signal was done in .mat files
...
if ANGPAIR_AVGS:
    ...
    angPairAverageSets['dorsal'] = [np.array([0,1]), #left
                                    np.array([2,3])] #right
...
upsideDownData = -1 # If your data is upside down, change the sign of this 1.
#### Plotting constants
