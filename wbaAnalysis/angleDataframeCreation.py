...
def get_head_axis(expt = '2024_04_19_E4', cameraView = "dorsal", frameNumToPlot = 20, TEST = 0):
    ...
    xant = np.mean(df_x[markers[0]])
    yant = np.mean(df_y[markers[0]])
    xpost = np.mean(df_x[markers[1]])
    ypost = np.mean(df_y[markers[1]])
...
def compute_vdist(df_x,df_y,frameNumsToComputeAngle,cfg):
    # HARD CODED HARD CODED HARD CODED HARD CODED HARD CODED HARD CODED HARD CODED
    translationalPairs = [(1,4), (11,14)] # LEFT ARISTA, LEFT H1 BASE, RIGHT ARISTA, RIGHT H1 BASE
    length = len(frameNumsToComputeAngle)
    vdist = np.zeros((length,len(translationalPairs)))
    for ii in range(len(translationalPairs)):
        pairind = translationalPairs[ii]
        bpind1 = pairind[0]
        bpind2 = pairind[1]
        for frameInd in range(len(frameNumsToComputeAngle)):
            frameNum = frameNumsToComputeAngle[frameInd]
...
def getAntennalData(experiment, cameraView, df):
    ...
    left_angles, right_angles, iaa, vert_dist = [], [], [], []
    l_idx = const.angPairs_byAntenna[cameraView][0]
    r_idx = const.angPairs_byAntenna[cameraView][1]
    for i in df.index:
        ...
        if const.ANGPAIR_AVGS == True:
            l_a = get_antenna_angles_adjusted(experiment, cameraView, j)[:,l_idx]
            r_a = get_antenna_angles_adjusted(experiment, cameraView, j)[:,r_idx]
    ...
    df['L_wing'] = left_angles
    df['R_wing'] = right_angles
    df['iaa'] = iaa
    return df
...
def get_vdist_conditionalAvgs(df):  
    if const.ONE_STIM == False:
        if const.MULT_SEQ == True:
            if const.FLIGHT:
                fnf = ['_Flight', '_noFlight']
                flightNames = [stim + '_vdist' + fnf[0] for stim in const.stimNames]
                noflightNames = [stim + '_vdist' + fnf[1] for stim in const.stimNames]
                for idx, name in enumerate(flightNames):
                    df[name] = pd.Series(dtype='object')
                    df[noflightNames[idx]] = pd.Series(dtype='object')
            for i in df.index:
    ...
    for condition in el.all_notes:
        ...
        if len(condition) > 0:
            for experiment in condition:
                ...
                if const.STIM: # Add stimulus Data
