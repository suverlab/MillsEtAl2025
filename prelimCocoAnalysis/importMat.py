...
def importMat(file_path = const.matDirectory):
    ...
    for ind in range(len(list_values[0,:])):
        ...
        if const.TRIM:
            pufferSignal.append(np.delete(expt[14], trim, axis=0))
            tachometerSignal.append(np.delete(expt[15], trim, axis=0))
        else:
            pufferSignal.append(expt[14][:])
            tachometerSignal.append(expt[15][:])
