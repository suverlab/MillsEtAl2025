...
def getConditionalAvgs(df):
    if const.ONLY_BASELINE == True:
        if const.BASELINE == True and const.FLIGHT == True:
            catgs = ['BaseAvg_Flight', 'BaseAvg_noFlight']
        ...
        for i in df.index:
            ...
            if const.FLIGHT == True and const.BASELINE == True:
                if np.all(flightBool): #full flight
                    ...
                    df[catgs[1]][i] = np.mean(traces[const.baselineStart*const.fps:baseEnd*const.fps,:], axis=0)
                else:
                    df[catgs[0]][i] = np.tile(np.nan,np.shape(traces)[1])
                    df[catgs[1]][i] = np.tile(np.nan,np.shape(traces)[1])
                if const.PUFFER:
                    if np.any(pufferSignal[const.baselineStart*const.fs:baseEnd*const.fs] > const.puffer_cutoff_value):
                        df[catgs[0]][i] = np.tile(np.nan,np.shape(traces)[1])
                        df[catgs[1]][i] = np.tile(np.nan,np.shape(traces)[1])
        
    elif const.ONE_STIM == False:
