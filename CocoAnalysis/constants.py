...
baseDirectory = '/Users/millskm/Desktop/Vanderbilt/SuverLab - Analysis/CocoAnalysis/' # Main Folder
matDirectory = baseDirectory + 'mat/' # Folder in which all .mat files are placed
...
lenVideo = 16 # Length of video in seconds for each trial
fps = 60 # Video Framerate
numFrames = lenVideo * fps # Total number of frames
fs = 20000 # Sampling Rate of DAQ
numSamples = lenVideo * fs

numTrials = 36 # put number of trial per fly here

TRIAL_NUMS_IN_MAT = True
if TRIAL_NUMS_IN_MAT == True:
    trial_num_name = 'trialnum' # If trial nums are available in mat file, those will be used. If not will iterate from 1 upwards.
...
# IF YOU PRE-TRIM in MATLAB, set to FALSE!
TRIM = False
if TRIM == True:
    trimStart = 0 # set to start time of excluded daq 
    trimLen = 2 # set to length time of excluded daq
...
# STIMULUS?
STIM = True # Set to True ONLY if there is an acitvely controlled stimulus in the experiment THAT YOU WANT AN AVERAGE FOR
if STIM == True: 
    BASELINE = True # Set to True if there is a baseline period, will take avg over said period and include in dataframe
    if BASELINE == True:
        baselineStart = 0 # set to start time of baseline period
        baselineLen = 4 # set to length time of baseline period
        baselineEnd = baselineStart + baselineLen
    
    ONLY_BASELINE = True # if you don't want a stimulus avg and just want the baseline, set this to true.
