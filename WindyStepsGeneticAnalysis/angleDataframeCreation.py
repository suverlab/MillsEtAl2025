...
def get_antenna_angles(expt = '2024_04_19_E4', cameraView = 'dorsal', trialNum = 3, TEST = 0):
    ...
    vdist = compute_vdist(df_x,df_y,frameNumsToComputeAngle,cfg)
...
def getAntennalData(experiment, cameraView, df):
    ...
    df['vdist'] =  pd.Series(dtype='object')
    ...
    for i in df.index:
        ...
        if const.ANGPAIR_AVGS == True:
            ...
            a, vdist = get_antenna_angles(experiment, cameraView, j)
            l_a = a[:,l_start_idx:l_end_idx+1]
            r_a = a[:,r_start_idx:r_end_idx+1]
        
        avg_vdist = (np.add(vdist[:,0],vdist[:,1])/2)
        vert_dist.append(avg_vdist)
        ...
        if len(condition) > 0:
            for experiment in condition:
                ...
                if const.STIM: # Add stimulus Data
                    ...
                    df_fly = get_vdist_conditionalAvgs(df_fly)
